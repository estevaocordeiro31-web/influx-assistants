...
import { 
  BookOpen, Mic, Star, CheckCircle2, XCircle, RotateCcw,
  ChevronLeft, ChevronRight, Zap, Target
} from "lucide-react";

// Dados das lessons do Book 5
const lessonsData = [
  {
    id: 1, title: "Friends and Acquaintances", unit: "Unit 1", theme: "Making Friends",
  ...
  {
    id: 2, title: "Friends and Acquaintances", unit: "Unit 1", theme: "Friends Forever",
    color: "from-purple-500 to-pink-500", chunks: []
  },
  {
    id: 3, title: "Family and Relationship", unit: "Unit 1", theme: "Family Ties",
  ...
  {
    id: 4, title: "Family and Relationship", unit: "Unit 1", theme: "Generations",
  ...
  {
    id: 5, title: "Shapes and Colors", unit: "Unit 2", theme: "Colors of My Day",
  ...
  {
    id: 6, title: "Shapes and Colors", unit: "Unit 2", theme: "Black and White",
...
function QuizSection({ lesson, onComplete }: { lesson: typeof lessonsData[0], onComplete: (score: number) => void }) {
  ...
  const questions = chunks.slice(0, 5).map((chunk, idx) => {
    ...
    return { question: `O que significa "${chunk.expression}"?`, options, correctAnswer: options.indexOf(chunk.meaning) };
  });
  ...
  if (showResult) {
    return (
      <div className="text-center py-6">
        <div className={`text-5xl mb-3 ${score >= 80 ? 'text-green-400' : score >= 60 ? 'text-yellow-400' : 'text-red-400'}`}>
          {score >= 80 ? '🏆' : score >= 60 ? '⭐' : '💪'}
        </div>
        ...
        <Button onClick={() => { setCurrentQuestion(0); setScore(0); setShowResult(false); setSelectedAnswer(null); setIsCorrect(null); }} className="bg-green-500 hover:bg-green-600">
        ...
        <div className="grid gap-2">
          {questions[currentQuestion].options.map((option, idx) => (
            <Button key={idx} variant="outline"
              className={`w-full justify-start p-3 h-auto text-left text-sm ${
                selectedAnswer === idx ? (isCorrect ? 'bg-green-500/20 border-green-500 text-green-400' : 'bg-red-500/20 border-red-500 text-red-400')
                : selectedAnswer !== null && idx === questions[currentQuestion].correctAnswer ? 'bg-green-500/20 border-green-500 text-green-400' : 'hover:bg-slate-700/50'
              }`}
              onClick={() => selectedAnswer === null && handleAnswer(idx)} disabled={selectedAnswer !== null}>
...
function FlashcardsSection({ lesson }: { lesson: typeof lessonsData[0] }) {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [flipped, setFlipped] = useState(false);

  const chunks = lesson.chunks;
  if (chunks.length === 0) {
    return <div className="text-center py-8"><p className="text-slate-400">Esta é uma lição comunicativa. Revise os chunks da lição anterior!</p></div>;
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="cursor-pointer" onClick={() => setFlipped(!flipped)}>
        {!flipped ? (
          <div className={`p-8 rounded-xl bg-gradient-to-br ${lesson.color} text-center`}>
          ...
          <div className="p-8 rounded-xl bg-slate-700 text-center">
            <p className="text-xl font-semibold text-green-400 mb-3">{currentChunk.meaning}</p>
            <p className="text-slate-300 italic text-sm">"{currentChunk.example}"</p>
          </div>
        )}
      </div>
      <div className="flex justify-center gap-3">
        <Button variant="outline" size="sm" onClick={() => { setFlipped(false); setCurrentIndex(Math.max(0, currentIndex - 1)); }} disabled={currentIndex === 0}>
          <ChevronLeft className="w-4 h-4" />
        </Button>
        <Button variant="outline" size="sm" onClick={() => { setFlipped(false); setCurrentIndex(Math.min(chunks.length - 1, currentIndex + 1)); }} disabled={currentIndex === chunks.length - 1}>
...
// Componente de Gravação de Pronúncia
function RecordSection({ lesson }: { lesson: typeof lessonsData[0] }) {
  ...
  const chunks = lesson.chunks;
  if (chunks.length === 0) {
    return <div className="text-center py-8"><p className="text-slate-400">Esta é uma lição comunicativa. Pratique a pronúncia dos chunks anteriores!</p></div>;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="p-6 bg-slate-900/50 rounded-lg text-center">
        ...
        <div className="flex justify-center gap-3 mb-4">
          {!isRecording ? (
            <Button className="bg-red-500 hover:bg-red-600" onClick={startRecording}><Mic className="w-4 h-4 mr-2" />Gravar</Button>
          ) : (
            <Button variant="outline" className="border-red-500 text-red-400 animate-pulse" onClick={stopRecording}>
              <div className="w-2 h-2 bg-red-500 rounded-full mr-2" />Parar
            </Button>
          )}
        </div>
        {audioUrl && (
          <div className="p-3 bg-slate-800 rounded-lg">
            <p className="text-xs text-slate-400 mb-2">Sua gravação:</p>
            <audio controls src={audioUrl} className="w-full h-8" />
          </div>
        )}
      </div>
      <div className="flex justify-center gap-3">
        <Button variant="outline" size="sm" onClick={() => { setAudioUrl(null); setCurrentChunkIndex(Math.max(0, currentChunkIndex - 1)); }} disabled={currentChunkIndex === 0}>
          <ChevronLeft className="w-4 h-4" />
        </Button>
        <Button variant="outline" size="sm" onClick={() => { setAudioUrl(null); setCurrentChunkIndex(Math.min(chunks.length - 1, currentChunkIndex + 1)); }} disabled={currentChunkIndex === chunks.length - 1}>
          <ChevronRight className="w-4 h-4" />
        </Button>
      </div>
    </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700">
        <CardHeader className="pb-3">
          <CardTitle className="text-white flex items-center gap-2 text-lg">
            <Target className="w-5 h-5 text-green-400" />Praticar Lessons - Book {currentBookId}
          </CardTitle>
        ...
        <CardContent>
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {lessonsData.map((lesson) => (
              <Button key={lesson.id} variant={selectedLesson.id === lesson.id ? "default" : "outline"}
                className={`h-auto py-2 px-3 ${selectedLesson.id === lesson.id ? `bg-gradient-to-r ${lesson.color} text-white border-0` : 'border-slate-600'}`}
                onClick={() => setSelectedLesson(lesson)}>
                <div className="text-center">
                  <div className="font-bold text-sm">L{lesson.id}</div>
                  <div className="text-[10px] opacity-80">{lesson.chunks.length} chunks</div>
                </div>
              </Button>
            ))}
          </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-white text-lg">Lesson {selectedLesson.id}: {selectedLesson.theme}</CardTitle>
              <CardDescription className="text-slate-400">{selectedLesson.unit} - {selectedLesson.title}</CardDescription>
            </div>
        ...
        <CardContent>
          <Tabs defaultValue="flashcards" className="space-y-4">
            <TabsList className="grid grid-cols-3 bg-slate-900/50">
              <TabsTrigger value="flashcards" className="data-[state=active]:bg-blue-500/20 text-xs sm:text-sm">
                <BookOpen className="w-4 h-4 mr-1 sm:mr-2" /><span className="hidden sm:inline">Flashcards</span><span className="sm:hidden">Cards</span>
              </TabsTrigger>
              <TabsTrigger value="quiz" className="data-[state=active]:bg-green-500/20 text-xs sm:text-sm">
                <Target className="w-4 h-4 mr-1 sm:mr-2" />Quiz
              </TabsTrigger>
              <TabsTrigger value="speak" className="data-[state=active]:bg-red-500/20 text-xs sm:text-sm">
                <Mic className="w-4 h-4 mr-1 sm:mr-2" /><span className="hidden sm:inline">Pronúncia</span><span className="sm:hidden">Falar</span>
              </TabsTrigger>
            </TabsList>
            <TabsContent value="flashcards"><FlashcardsSection lesson={selectedLesson} /></TabsContent>
            <TabsContent value="quiz"><QuizSection lesson={selectedLesson} onComplete={(score) => setTotalPoints(prev => prev + score)} /></TabsContent>
            <TabsContent value="speak"><RecordSection lesson={selectedLesson} /></TabsContent>
          </Tabs>
        </CardContent>
      </Card>
    </div>
